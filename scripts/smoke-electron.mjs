...
const smokeProfile = path.join(smokeDir, 'profile-' + Date.now());
for (const folder of ['user-data', 'cache', 'documents']) {
  mkdirSync(path.join(smokeProfile, folder), { recursive: true });
}
const smokeNovel = process.env.CHARACTER_GRAPH_SMOKE_IMPORT_PATH || path.join(smokeDir, 'novel-' + Date.now() + '.txt');
...
function finishFromOutput(code = 0) {
  ...
  if (!result.apiReady || !result.projectCreated || result.chapterCount < 2) {
    console.error('Smoke failed: ' + JSON.stringify({ result }, null, 2));
    stopChild();
    process.exit(1);
  }
  stopChild();
...
child.on('exit', (code) => {
  ...
  const match = output.match(/\[smoke\](\{.*\})/);
  if (!match) {
    console.error('Smoke result was not emitted.');
    process.exit(1);
  }
